...
angular.module('com.module.products')
  .run(function ($rootScope, Product, Category, gettextCatalog) {
    ...
    Product.find({filter:{include:['category']}},function (data) {
		...
		$rootScope.addDashboardBox(gettextCatalog.getString('Products'), 'bg-yellow', 'ion-ios7-cart-outline',{name:'products_count',quantity: data.length}, 'app.products.list',false);
    });
    Category.find({filter:{where:{'status':1}}}, function (data) {
      $rootScope.addDashboardBox(gettextCatalog.getString('Categories'), 'bg-aqua', 'ion-ios7-pricetag-outline',{name:'categories_count',quantity: data.length}, 'app.products.list',false);
    });
  });